...
carbon_format(Prefix, Host, Metrics) ->
    ...
    lists:flatten([carbon_string(Prefix, Host, Metric, Timestamp) || Metric <- Metrics]).

% private
-spec carbon_string(binary(), binary(), read_metric(), integer()) -> carbon_string() | carbon_batch().
      ...
      _ ->
          case Type of
            ...
            gauge ->
                carbon_string(MetricKey, integer_to_binary(Value), TimestampBin);
            dist ->
                [carbon_string(<<MetricKey/binary, ".", (atom_to_binary(Key, latin1))/binary>>,
                               integer_to_binary(Val),
                               TimestampBin)
                 || {Key, Val} <- maps:to_list(Value)];
            histo ->
                #{n_samples := NSamples, percentiles := Percentiles} = Value,
                [carbon_string(<<MetricKey/binary, ".n_samples">>, integer_to_binary(NSamples), TimestampBin)
                 | [carbon_string(<<MetricKey/binary, ".", Key/binary>>, Val, TimestampBin)
