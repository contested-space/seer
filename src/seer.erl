...
-spec read_all() -> list({metric_type(), metric_name(), read_value()}).
read_all() ->
...
-spec get_or_new(metric_type(), metric_name()) ->
...
-spec get(metric_type(), metric_name()) ->
    {ok, atomics:atomics_ref()} | {error, term()}.
...
-spec histo_percentiles(
    ...
    #{histo_percentile() => histo_bucket_key()} | #{}.
histo_percentiles(_N, _Buckets, [], Res) -> Res;
histo_percentiles(
    N,
    ...
    case PercentileIdx >= LoIdx andalso PercentileIdx =< HiIdx of
        ...
        false -> histo_percentiles(LoIdx, BsRest, Ps, Res)
    end.
...
-spec nths(dist_stats_indexes(), integer(), list(integer()), dist_stats()) ->
    dist_stats().
...
-spec read_metric(metric_type(), metric_name(), atomics:atomics_ref()) ->
    read_value().
